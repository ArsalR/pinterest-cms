...
export const SITE_SCHEMA_STATEMENTS: readonly string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS seo_settings (id TEXT PRIMARY KEY DEFAULT 'default', block_ai_bots INTEGER NOT NULL DEFAULT 0, blocked_bots TEXT, disallow_paths TEXT, robots_extra TEXT, rss_enabled INTEGER NOT NULL DEFAULT 1, archives_enabled INTEGER NOT NULL DEFAULT 1, global_schema_enabled INTEGER NOT NULL DEFAULT 0, org_name TEXT, org_logo TEXT, social_profiles TEXT, profiles TEXT, scripts TEXT, indexnow_key TEXT, merchant_json TEXT, image_license_json TEXT, analytics_enabled INTEGER NOT NULL DEFAULT 0, analytics_key TEXT, pixel_consent INTEGER, bing_verify TEXT, updated_at TEXT DEFAULT (datetime('now')))`,
  // V1.3 Local SEO profile — NAP once per location, multi-location. Additive.
  `CREATE TABLE IF NOT EXISTS business_locations (id TEXT PRIMARY KEY, name TEXT NOT NULL, subtype TEXT, street TEXT, city TEXT, region TEXT, postal TEXT, country TEXT, phone TEXT, hours_json TEXT, latitude REAL, longitude REAL, service_areas TEXT, price_range TEXT, gbp_url TEXT, rating_value REAL, rating_count INTEGER, is_primary INTEGER NOT NULL DEFAULT 0, slug TEXT UNIQUE, created_at TEXT DEFAULT (datetime('now')))`,
  // V1.3 News SEO profile — authors (E-E-A-T backbone). Additive.
  `CREATE TABLE IF NOT EXISTS authors (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT UNIQUE NOT NULL, bio TEXT, photo TEXT, same_as TEXT, created_at TEXT DEFAULT (datetime('now')))`,
  // V1.4 Forms Engine. Additive — empty = no forms.
  `CREATE TABLE IF NOT EXISTS forms (id TEXT PRIMARY KEY, slug TEXT UNIQUE NOT NULL, title TEXT NOT NULL, fields_json TEXT NOT NULL, ack_enabled INTEGER NOT NULL DEFAULT 0, ack_subject TEXT, ack_body TEXT, webhook_url TEXT, webhook_secret TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))`,
  `CREATE TABLE IF NOT EXISTS form_submissions (id TEXT PRIMARY KEY, form_id TEXT NOT NULL, fields_json TEXT NOT NULL, page TEXT, country TEXT, status TEXT NOT NULL DEFAULT 'new', notes TEXT, thread_json TEXT, ai_summary TEXT, ai_score TEXT, created_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (form_id) REFERENCES forms(id) ON DELETE CASCADE)`,
  `CREATE TABLE IF NOT EXISTS subscribers (id TEXT PRIMARY KEY, email TEXT UNIQUE NOT NULL, confirmed INTEGER NOT NULL DEFAULT 0, confirm_token TEXT, unsubscribed INTEGER NOT NULL DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`,
  `CREATE INDEX IF NOT EXISTS idx_form_submissions_form ON form_submissions(form_id, created_at)`,
  // Site Mailbox (V1.5 M1) — mirror of migrate.ts v16. Additive; inert with no mailbox.
  `CREATE TABLE IF NOT EXISTS mail_addresses (id TEXT PRIMARY KEY, address TEXT UNIQUE NOT NULL, label TEXT, is_catch_all INTEGER NOT NULL DEFAULT 0, active INTEGER NOT NULL DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))`,
  `CREATE TABLE IF NOT EXISTS mail_messages (id TEXT PRIMARY KEY, thread_key TEXT NOT NULL, direction TEXT NOT NULL DEFAULT 'in', from_addr TEXT NOT NULL, to_addr TEXT NOT NULL, subject TEXT, body_text TEXT, body_html TEXT, message_id TEXT, in_reply_to TEXT, refs TEXT, attachments_json TEXT, status TEXT NOT NULL DEFAULT 'new', spam INTEGER NOT NULL DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`,
