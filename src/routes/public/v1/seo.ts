...
seoRoutes.get("/", async (c) => {
  ...
  const r = await siteDb.execute({
    sql: `SELECT id, slug, sitemap_exclude, nofollow, schema_type, faq_json, author_id, llms_exclude, seo_keywords
  ...
  const seo = r.rows.map((row) => ({
    ...
    llmsExclude: Number(row.llms_exclude) === 1,
    focusKeyword: (row.seo_keywords as string | null) ?? null,
