...
export const dashboardHandler = async (c: Context<AppEnv>): Promise<Response> => {
  try {
    ...
    if (!siteDb) {
      console.error("[dashboard] siteDb is null — tenantMiddleware may not have run")
      return c.html(errorPage("Database not available. Please try again."), 500)
    }

    const settings = await loadSettings(siteDb)

    // Core stats — these tables always exist.
    const [postCount, publishedCount, draftCount, apiCount, mediaCount, categoryCount, recent] =
      await Promise.all([
    ...
    // api_logs may not exist on older sites — query it separately and fall back silently.
    let apiLogsRows: Record<string, unknown>[] = []
    try {
      const apiLogs = await siteDb.execute(`
      ...
      `)
      apiLogsRows = apiLogs.rows as Record<string, unknown>[]
    } catch (e) {
      console.error("[dashboard] api_logs query failed (table may not exist yet):", e)
    }
    ...
    const apiLogsHtml = apiLogsRows.length
      ? `<table>
...
function errorPage(message: string): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Dashboard error</title>
  <style>body{font-family:sans-serif;padding:60px;background:#111;color:#eee}
  pre{background:#222;padding:16px;border-radius:6px;overflow:auto;font-size:13px;color:#fca5a5}</style></head>
  <body><h1>Dashboard error</h1>
  <p>Something went wrong loading the dashboard. Check <code>wrangler tail</code> for the full error.</p>
  <pre>${safe}</pre>
  <p><a href="/admin/login" style="color:#60a5fa">← Back to login</a></p>
  </body></html>`
}
