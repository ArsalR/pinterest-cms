...
import { mailboxUnread } from "../mail"
import {
...
export async function siteDetailHandler(c: Context<AppEnv>): Promise<Response> {
  ...
  // V1.5 M1: unread badge on the Mailbox button.
  const unreadMail = site.cms_site_id ? await mailboxUnread(db, site.cms_site_id).catch(() => 0) : 0
  const url = new URL(c.req.url)
  const error = url.searchParams.get("error")
  ...
  const body = `${SITES_STYLES}
    ...
    <div class="card">
      ...
      ${site.status === "active" ? `<a class="btn" href="https://${escapeAttr(canonicalHost)}/" target="_blank">Open site ↗</a>
        ...
        <a class="btn ghost" href="/app/sites/${escapeAttr(siteId)}/cleanup">Clean up imports</a>
        <a class="btn ghost" href="/app/sites/${escapeAttr(siteId)}/clone">Clone</a>
        <form method="POST" action="/app/sites/${escapeAttr(siteId)}/reseed" style="display:inline" onsubmit="return confirm('Re-theme and re-seed this site with Claude? Best for a freshly-cloned site.')"><button class="btn ghost" type="submit">Re-theme &amp; re-seed</button></form>
