...
export async function loadPostSeo(master: Client, cmsSiteId: string, postId: string): Promise<PostSeoRow | null> {
  ...
  const r = await siteDb.execute({
    sql: `SELECT id, title, slug, published, excerpt, cover_image, content, seo_keywords, seo_title, seo_description, author_id, llms_exclude,
  ...
  return {
    ...
    llmsExclude: Number(p.llms_exclude) === 1,
  }
}
...
export async function savePostSeo(env: CloudflareEnv, customerId: string, cmsSiteId: string, repoFullName: string | null, postId: string, u: SeoUpdate, master: Client): Promise<SaveResult> {
  ...
  await siteDb.execute({
    sql: `UPDATE posts SET seo_title=?, seo_description=?, seo_keywords=?, slug=?, og_title=?, og_description=?, og_image=?,
             canonical_url=?, no_index=?, sitemap_exclude=?, nofollow=?, schema_type=?, faq_json=?, author_id=?, llms_exclude=?, updated_at=datetime('now')
          WHERE id=?`,
    args: [
      ...
      u.schemaType || null, faqJson.length ? JSON.stringify(faqJson) : null, u.authorId.trim() || null, u.llmsExclude ? 1 : 0, postId,
    ],
  })
