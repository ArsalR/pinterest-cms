...
6. **Cloudflare stays FREE TIER for now**: crypto parameters config-driven and tuned to fit the 10ms CPU budget (upgrade strengthens without data migration); uptime checks deferred/reduced until paid.
   - **UPDATE (post-audit, 2026-07): upgraded to Workers Paid ($5/mo)** after the Part E4 measurement showed provisioning (~100 subrequests/invocation) exceeds the free-tier 50-subrequest / 10ms-CPU ceiling. Paid raises these to 1000 subrequests / 30s CPU, so provisioning runs as-is. Follow-ups landed on the `paid-tier` branch: (a) customer PBKDF2 raised 100k → **600k** (OWASP-2023) via the config-driven envelope — existing customers rehash-on-login, no migration; (b) **uptime monitoring un-deferred** at the 5-minute cadence (`analytics/uptime.ts`, rides the `*/5` cron); (c) API-key PBKDF2 reduction (E4-C) intentionally NOT done — the CPU saving is unnecessary on paid and would touch the frozen verify path. The free-tier hygiene fixes from #41 (schema batch, cron caps) stay — they're good regardless of tier.
