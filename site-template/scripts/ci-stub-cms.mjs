...
createServer((req, res) => {
  ...
  if (url.includes("/seo-settings")) {
    return res.end(JSON.stringify({ success: true, settings: {
      ...
      profiles: PROFILES, scripts: process.env.STUB_SCRIPTS === "overbudget" ? [{ id: "ga4", config: "G-ABC123XYZ" }, { id: "crisp", config: "12345678-abcd-ef01-2345-6789abcdef01" }, { id: "cookieyes", config: "abcdef123456" }] : process.env.STUB_SCRIPTS === "light" ? [{ id: "plausible", config: "example.com" }] : process.env.STUB_PIXELS === "all" ? [{ id: "meta_pixel", config: "123456789012345" }, { id: "google_ads", config: "AW-123456789" }, { id: "tiktok_pixel", config: "C1234567890123456789" }, { id: "linkedin_insight", config: "1234567" }, { id: "pinterest_tag", config: "2612345678901" }] : process.env.STUB_PIXELS === "1" ? [{ id: "meta_pixel", config: "123456789012345" }, { id: "pinterest_tag", config: "2612345678901" }] : [], indexnowKey: PROFILES.includes("news") ? "a".repeat(32) : "", analyticsEnabled: process.env.STUB_ANALYTICS === "1", analyticsKey: process.env.STUB_ANALYTICS === "1" ? "sitetoken123" : "", pixelConsent: null, bingVerify: process.env.STUB_BING === "1" ? "ABC123BINGVERIFY" : "",
